...
In this fork of the repository I fine-tuned the Kalman filter by reducing the standard deviation (std) value asociated with measurements' (detections) aspect ratio, in the project method from kalman_filter.py. By doing this, I reduced the expected uncertainty in measurements' aspect ratio, allowing the Kalman filter to adapt tracked bounding box's aspect ratio based on subsequent measurements.

This solution aimed to solve the partial appearance of objects in an initial frame, which led to inaccurate aspect ratio for detections' bounding boxes. The original value for that std propagates the initial aspect ratio encountered (corresponding to the part of the object appearing in the first frame) through the following frames even if after a few frames the object become fully visible and its detection aspect ratio differ from the one tracked initially.
